...
#set page(
  ...
  footer: context [
    ...
    #grid(columns: (1fr, auto), align: (left, right),
      [risk-engine-ca], [#counter(page).display("1 / 1", both: true)])
  ],
)
...
#align(center)[
  #block(width: 100%)[
    ...
    #v(0.6em)
    #text(size: 10pt, fill: luma(70))[Guillaume Vaudescal · 2026-08-29 · #link("https://github.com/Guilou001/06-risque-marche")[Guilou001/06-risque-marche]]
  ]
]
#v(1.2em)
...
+ *Historique* : le quantile empirique des 500 derniers jours, tel quel.
+ *Gaussien* : moyenne et écart-type de la fenêtre, quantile de la loi normale.
+ *Student-t* : une loi de Student, dont les queues épaisses donnent plus de poids aux jours extrêmes, ajustée par maximum de vraisemblance et réajustée tous les 21 jours.
+ *EWMA* : la volatilité RiskMetrics, une moyenne des carrés de rendements qui oublie exponentiellement le passé (lambda = 0,94), avec un quantile normal.
+ *Simulation historique filtrée (FHS)* : chaque rendement passé est divisé par la volatilité EWMA de son jour, puis remis à l'échelle de la volatilité prévue pour demain ; le quantile est pris sur ces rendements re-scalés. La queue reste empirique, le niveau suit la volatilité.
+ *GARCH(1,1)* : la variance de demain est une combinaison estimée de la variance d'aujourd'hui et du choc d'aujourd'hui, réestimée tous les 21 jours par maximum de vraisemblance, innovations normales.
...
Tous les chiffres viennent de #raw("results/tables/backtests_var99.csv") et #raw("backtests_es975.csv"), régénérés par #raw("uv run rke backtest") ; 5 476 jours communs, novembre 2004 à août 2026, VaR à 99 %.
...
#figure(image("../results/figures/cone_var.png", width: 100%), caption: [Cône de VaR et dépassements])
...
#figure(image("../results/figures/feux_bale.png", width: 100%), caption: [Feux tricolores de Bâle])
...
#figure(image("../results/figures/reactivite_2020.png", width: 100%), caption: [Réactivité pendant mars 2020])
...
#raw("uv sync --locked --all-extras     # environnement verrouillé (Python 3.12, scipy, sans bibliothèque GARCH)\nuv run pytest                     # 10 tests synthétiques, sans réseau (quantiles, fuite, GARCH, juges)\nuv run rke fetch                  # prix quotidiens des six FNB (quelques secondes, non commités)\nuv run rke backtest               # 6 modèles x 5 476 jours x 2 niveaux : tables + 3 figures (12 s mesurées)", block: true, lang: "bash")
...
#table(
  ...
  inset: 5pt,
    [*Limite*],
    [*Statut*],
    [Le GARCH utilise des innovations normales ; un GARCH-t ou un GARCH-FHS ferait mieux en queue],
    [reconnu ; la FHS joue déjà ce rôle avec la volatilité EWMA],
    [La statistique Z2 est rapportée sans p-valeur simulée (Acerbi-Székely la calculent par simulation du modèle) ; le seuil indicatif de −0,7 est un précepte, pas une mesure],
    [déclaré],
    [Poids du portefeuille tenus constants aux cibles (la rotation réelle mesurée au dépôt 03 est de 2,8 % par an)],
    [approximation déclarée],
    [Un seul portefeuille testé ; les conclusions valent pour un profil équilibré, pas pour un livre de dérivés],
    [reconnu],
    [Fenêtre unique de 500 jours et réajustement tous les 21 jours, non optimisés],
    [choix déclarés],
    [Classeur Excel/VBA de restitution pour comité de risque],
    [à venir],
)

== 8. Crédits, licence, citation

Kupiec, P. (1995), « Techniques for Verifying the Accuracy of Risk Measurement Models » ; Christoffersen, P. (1998), « Evaluating Interval Forecasts » ; Barone-Adesi, G., Giannopoulos, K. et Vosper, L. (1999), « VaR without Correlations for Portfolios of Derivative Securities » ; Bollerslev, T. (1986) ; Acerbi, C. et Székely, B. (2014), « Backtesting Expected Shortfall » ; J.P. Morgan (1996), RiskMetrics Technical Document ; Comité de Bâle (1996, 2019). Données : Yahoo Finance, usage personnel, non redistribuées. Code : Guillaume Vaudescal, 2026, licence MIT.
